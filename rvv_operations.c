...
void fill_matrix_lmul2(int *H, const int8_t *seq1, const int8_t *seq2, int rows, int cols, int *max_score) {
    *max_score = 0;
    vint32m1_t current_max_vec = __riscv_vmv_v_x_i32m1(0, 1);
    ...
    for (int d = 2; d <= rows + cols - 2; d++) {
        ...
        for (int offset = 0; offset < len; ) {
            ...
            vint32m2_t vec_tmp = __riscv_vmax_vv_i32m2(
...
void fill_matrix_lmul4(int *H, const int8_t *seq1, const int8_t *seq2, int rows, int cols, int *max_score) {
    *max_score = 0;
    vint32m1_t current_max_vec = __riscv_vmv_v_x_i32m1(0, 1);
    ...
    for (int d = 2; d <= rows + cols - 2; d++) {
        ...
        for (int offset = 0; offset < len; ) {
            ...
            for (int k = 0; k < vl; k++) {
                ...
                temp_up[k] = H[(i-1)*cols + j];
                temp_left[k] = H[i*cols + (j-1)];
                temp_blosum[k] = (int)iBlosum62[seq1[i - 1] * 26 + seq2[j - 1]];
            }
            ...
            vint32m4_t vec_tmp = __riscv_vmax_vv_i32m4(
                __riscv_vmax_vv_i32m4(vec_match, vec_del, vl),
                vec_ins, vl
            );
...
void fill_matrix_lmul8(int *H, const int8_t *seq1, const int8_t *seq2, int rows, int cols, int *max_score) {
    *max_score = 0;
    vint32m1_t current_max_vec = __riscv_vmv_v_x_i32m1(0, 1);
